...
async function checkInvites() {
  const { count, error } = await supabase.from('email_invites').select('*', { count: 'exact', head: true });
  console.log(`Invites: ${count}`);
  
  const { data } = await supabase.from('email_invites').select('*').limit(5);
  console.log('Invites sample:', data);
}
